...
const EpisodePagiNation = ({
  ...
  return (
    <div
      className={`${theme.text.selected} relative flex flex-row text-gray-200 w-full justify-end items-center p-1 px-3`}
    >
      <span className=" absolute left-1 p-1 font-black text-[20px]">
        EP {page}
      </span>
      <div
        className={`group relative p-2 shadow-lg rounded-sm   font-bold cursor-pointer  hover:text-yellow-500`}
        onClick={light}
      >
        <FaLightbulb size={24} strokeWidth={1} className="" />
        <span className="hidden group-hover:lg:block  bg-[#1e1b1bc7] text-gray-200 px-[12px] py-[2px] absolute top-[-32px] left-[-1rem]  rounded-md">
          Light
        </span>
      </div>

      <div
        className={`group relative p-2 shadow-lg rounded-sm   font-bold cursor-pointer  hover:text-blue-600`}
        onClick={change}
      >
      ...
      >
        <a
          href={`https://goload.io/download?id=${epId}`}
          rel="noreferrer"
          target="_blank"
          className=""
        >
          <FaDownload size={20} className=" hover:text-blue-600" />
          <span className="hidden group-hover:lg:block  bg-[#1e1b1bc7] text-gray-200 px-[12px] py-[2px] absolute top-[-32px] left-[-2rem]  rounded-md">
            Download
          </span>
        </a>
      </div>
